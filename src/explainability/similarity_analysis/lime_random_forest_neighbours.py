"""
RQ2, part 2 - Random Forest, LIME similarity stability. Same approach as
...
def main():
    ...
    anchor_explanations = pd.read_csv(EXPLAIN_DIR / "lime_random_forest.csv").set_index("SK_ID_CURR")
    ...
    neighbor_explanations.to_csv(SIMILARITY_DIR / "lime_random_forest_neighbors.csv")
    print(f"\nSaved neighbor explanations to {SIMILARITY_DIR / 'lime_random_forest_neighbors.csv'}")
    ...
    scores_path = SIMILARITY_DIR / "stability_scores_lime_random_forest.csv"
    results_df.to_csv(scores_path, index=False)
    print(f"Saved per-anchor stability scores to {scores_path}")

    print(f"\nRandom Forest - LIME similarity stability")
