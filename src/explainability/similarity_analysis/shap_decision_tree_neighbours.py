"""
RQ2, part 2 - Decision Tree, SHAP similarity stability. Same approach as
the Logistic Regression version.
"""
...
def main():
    ...
    anchor_explanations = pd.read_csv(EXPLAIN_DIR / "shap_decision_tree.csv").set_index("SK_ID_CURR")
    ...
    neighbor_explanations.to_csv(SIMILARITY_DIR / "shap_decision_tree_neighbors.csv")
    print(f"Saved neighbor explanations to {SIMILARITY_DIR / 'shap_decision_tree_neighbors.csv'}")
    ...
    scores_path = SIMILARITY_DIR / "stability_scores_shap_decision_tree.csv"
    results_df.to_csv(scores_path, index=False)
    print(f"Saved per-anchor stability scores to {scores_path}")

    print(f"\nDecision Tree - SHAP similarity stability")
