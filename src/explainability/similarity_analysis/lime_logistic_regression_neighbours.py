"""
RQ2, part 2 - Logistic Regression, LIME similarity stability. Same idea
as the SHAP version, but one applicant at a time via LIME. Needs the
scaler, same reasoning as every other Logistic Regression script.
"""
...
def main():
    ...
    anchor_explanations = pd.read_csv(EXPLAIN_DIR / "lime_logistic_regression.csv").set_index("SK_ID_CURR")
    ...
    explainer = LimeTabularExplainer(
        training_data=X_train_scaled, feature_names=feature_cols,
    ...
    all_rows = []
    for i, row in enumerate(X_neighbors_scaled):
    ...
    neighbor_explanations.to_csv(SIMILARITY_DIR / "lime_logistic_regression_neighbors.csv")
    print(f"\nSaved neighbor explanations to {SIMILARITY_DIR / 'lime_logistic_regression_neighbors.csv'}")
    ...
    scores_path = SIMILARITY_DIR / "stability_scores_lime_logistic_regression.csv"
    results_df.to_csv(scores_path, index=False)
    print(f"Saved per-anchor stability scores to {scores_path}")

    print(f"\nLogistic Regression - LIME similarity stability")
