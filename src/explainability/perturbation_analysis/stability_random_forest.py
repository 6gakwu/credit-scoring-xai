"""
RQ1, part 2 - Random Forest perturbation stability.

Same approach as the other SHAP stability scripts.
Given Random Forest's known probability compression (from model
training), expect these raw contribution numbers to be smaller than
the other models
"""
...
def main():
    ...
    original_explanations = pd.read_csv(EXPLAIN_DIR / "shap_random_forest.csv")
    ...
    raw_output_path = PERTURB_DIR / "shap_random_forest_perturbed.csv"
    ...
    scores_path = PERTURB_DIR / "stability_scores_shap_random_forest.csv"
    scores_df.to_csv(scores_path, index=False)
    print(f"Saved per-applicant stability scores to {scores_path}")

    print(f"\nRandom Forest - SHAP perturbation stability")
