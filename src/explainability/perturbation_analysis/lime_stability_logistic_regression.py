"""
RQ1, part 2 - Logistic Regression LIME perturbation stability.

Same idea as the SHAP versions, but using LIME instead - one applicant
at a time, 250 total this round instead of 200.
"""
...
def main():
    ...
    original_explanations = pd.read_csv(EXPLAIN_DIR / "lime_logistic_regression.csv")
    ...
    all_rows = []
    for i, row in enumerate(X_perturbed_scaled):
    ...
    raw_output_path = PERTURB_DIR / "lime_logistic_regression_perturbed.csv"
    ...
    scores_path = PERTURB_DIR / "stability_scores_lime_logistic_regression.csv"
    scores_df.to_csv(scores_path, index=False)
    print(f"Saved per-applicant stability scores to {scores_path}")

    print(f"\nLogistic Regression - LIME perturbation stability")
