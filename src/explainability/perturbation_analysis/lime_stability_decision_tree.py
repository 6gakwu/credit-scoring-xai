"""
RQ1, part 2 - Decision Tree LIME perturbation stability. Same approach
as the Logistic Regression LIME version.
"""
...
def main():
    ...
    original_explanations = pd.read_csv(EXPLAIN_DIR / "lime_decision_tree.csv")
    ...
    raw_output_path = PERTURB_DIR / "lime_decision_tree_perturbed.csv"
    ...
    scores_path = PERTURB_DIR / "stability_scores_lime_decision_tree.csv"
    scores_df.to_csv(scores_path, index=False)
    print(f"Saved per-applicant stability scores to {scores_path}")

    print(f"\nDecision Tree - LIME perturbation stability")
