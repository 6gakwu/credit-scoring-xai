"""
RQ1, part 2 - XGBoost LIME perturbation stability. Same approach as the
other LIME stability scripts. 
"""
...
def main():
    ...
    original_explanations = pd.read_csv(EXPLAIN_DIR / "lime_xgboost.csv")
    ...
    raw_output_path = PERTURB_DIR / "lime_xgboost_perturbed.csv"
    ...
    scores_path = PERTURB_DIR / "stability_scores_lime_xgboost.csv"
    scores_df.to_csv(scores_path, index=False)
    print(f"Saved per-applicant stability scores to {scores_path}")

    print(f"\nXGBoost - LIME perturbation stability")
